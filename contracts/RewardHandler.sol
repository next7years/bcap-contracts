...
contract RewardHandler is Ownable, AccessControl, ReentrancyGuard, Pausable {
  ...
  /// @dev Tracks the total supply of the minted BCAPs
  uint256 private _totalSupply;

  /// @dev Tracks the amount of BCAP minted per user
  mapping(address => uint256) private _balances;

  /// @notice An event emitted when a reward is added
  event RewardAdded(uint256 reward);

  /// @notice An event emitted when BCAP is minted and staked to earn rewards
  event Staked(address indexed user, uint256 amount);

  /// @notice An event emitted when BCAP is burned and removed of stake
   ...
   * gyu: this function is run whenever it's get called to update the percentage of the reward per token value
   */
  ...
  /// @notice Returns the total amount of BCAP tokens minted and getting reward on this vault.
  function totalSupply() external view returns (uint256) {
    return _totalSupply;
  }

  /**
   * @notice Returns the amount of BCAP tokens minted and getting reward from specific user.
   * @param _account address
   */
  ...
  /**
   * @notice Called when BCAP is minted, adds the minted value as stake
  ...
  /**
   * @notice Called when BCAP is burned or liquidated, removes the burned value as stake
   * @param _staker address
   * @param _amount uint
   * @dev Only vault can call it
   * @dev Updates rewards on call
  ...
  /**
   * @notice Called when BCAP is burned or liquidated, transfers to the staker the current amount of rewards tokens earned.
   * @param _staker address
   * @dev Only vault can call it
   * @dev Updates rewards on call
   */
